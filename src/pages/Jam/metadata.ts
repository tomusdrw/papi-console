...
h.encode = (e: jamCodec.Encoder, value: any) => {
  ...
  if (!isOk) {
    ...
    kind = m[value.type] || workResult.WorkExecResultKind.panic
  }
...
export function createMetadata(spec: config.ChainSpec) {
  ...
  const epochMarkerValidatorsArr = add(
    codec.sequenceFixLen(hash.Codec, spec.validatorsCount),
    {
      type: "array",
  ...
  const ticketsMarkerArray = add(
    codec.sequenceFixLen(ticket.Codec, spec.epochLength),
    {
      type: "array",
  ...
  const assuranceBitfield = add(codec.bitVecFixLen(assurancesBitfieldBytes), {
    type: "array",
    len: assurancesBitfieldBytes / 8,
    value: u8,
  })

  const availabilityAssurance = addStruct(
  ...
  const verdict = addStruct(
    disputes.Verdict,
    {
      workReportHash: hash,
      votesEpoch: u32,
      votes: add(
        codec.sequenceFixLen(judgement.Codec, spec.validatorsSuperMajority),
        {
          type: "array",
          value: judgement,
  ...
  const disputesExtrinsic = addStruct(
  ...
  const workPackageSpec = addStruct(
  ...
  const segmentRootLookupItem = addStruct(
    workReport.SegmentRootLookupItem,
    {
  ...
  const workItemExtrinsicSpec = addStruct(
  ...
  const report = addStruct(
    workReport.WorkReport,
    {
      ...
      segmentRootLookup: add(
        codec.sequenceVarLen(workReport.SegmentRootLookupItem.Codec),
        {
          type: "sequence",
  ...
  const reportGuarantee = addStruct(
  ...
  const guaranteesExtrinsic = add(
    gaurantees.guaranteesExtrinsicCodec,
    {
      type: "sequence",
  ...
  const preimagesExtrinsic = add(
    preimage.preimagesExtrinsicCodec,
    {
      type: "sequence",
      value: pImage,
    },
