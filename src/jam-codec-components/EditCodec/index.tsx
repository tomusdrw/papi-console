...
function encode(codec: jamCodec.Descriptor<unknown>, value: unknown): Uint8Array | undefined {
    try {
      let vJam = compatBinaryType(value, true);
      console.log('encoding', vJam, typeof vJam);
      return jamCodec.Encoder.encodeObject(codec, vJam).asOpaque();
    } catch (e) {
      console.log('Error while encoding', codec, value);
      console.warn(e);
      return undefined;
    }
}

function refresh(codec: jamCodec.Descriptor<unknown>, value: Uint8Array | HexString | undefined): CodecComponentUpdate {
...
export function createDecode<T>(cod: jamCodec.Codec<T>): DecodeFun<T> {
  return (bytes) => {
    let source = null;
    ...
    try {
      const result2 = jamCodec.Decoder.decodeObject(cod, source);
...
export function getJamCodecComponent(baseComponents: EditComponents) {
  ...
  function JamCodecComponent<T>({ entry, value, dynCodecs, onChange, path }: JamCodecComponentProps<T>) {
    ...
    if (!value.empty) {
      const tryEncode = () => {
        console.log('attempting to encode', value);
        return encode(cod, value.decoded);
      };
  ...
  return (<COption
    {
      ...{
        ...
        decode,
        inner: !value.empty && value.decoded === void 0 ? <CVoid /> : (<JamCodecComponent
          {...{
            path: path.concat(
      ...
      onChange: (x: T | NOTIN) => {
        ...
        if (!hasNotin) {
          ...
          try {
            ...
            console.warn("Error encoding", xJam, e);
            return false;
          }
        }
